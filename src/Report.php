...
class Report extends CommonGLPI
{
    ...
    public static function getReports(): array
    {
        ...
        $report_list["default"]["file"] =  $CFG_GLPI['root_doc'] ."/front/report.default.php";

        if (Contract::canView()) {
            $report_list["Contrats"]["name"] = __('By contract');
            $report_list["Contrats"]["file"] =  $CFG_GLPI['root_doc'] ."/front/report.contract.php";
        }
        if (Infocom::canView()) {
            $report_list["Par_annee"]["name"] = __('By year');
            $report_list["Par_annee"]["file"] =  $CFG_GLPI['root_doc'] ."/front/report.year.php";
            $report_list["Infocoms"]["name"]  = __('Hardware financial and administrative information');
            $report_list["Infocoms"]["file"]  =  $CFG_GLPI['root_doc'] ."/front/report.infocom.php";
            $report_list["Infocoms2"]["name"] = __('Other financial and administrative information (licenses, cartridges, consumables)');
            $report_list["Infocoms2"]["file"] =  $CFG_GLPI['root_doc'] ."/front/report.infocom.conso.php";
        }
        if (Session::haveRight("networking", READ)) {
            // Network socket report
            $report_list["Rapport prises reseau"]["name"] = __('Network report');
            $report_list["Rapport prises reseau"]["file"] =  $CFG_GLPI['root_doc'] ."/front/report.networking.php";
        }
        if (Session::haveRight("reservation", READ)) {
            $report_list["reservation"]["name"] = __('Loan');
            $report_list["reservation"]["file"] =  $CFG_GLPI['root_doc'] ."/front/report.reservation.php";
        }
        
        if (Session::haveRight("reports", READ)) {
            $report_list["TicketPerformance"]["name"] = __('Ticket Performance Report');
            $report_list["TicketPerformance"]["file"] =  $CFG_GLPI['root_doc'] ."/front/report.ticketperformance.php";
        }
        ...
        if (
            ...
            $report_list["state"]["file"] = $CFG_GLPI['root_doc'] ."/front/report.state.php";
        }
