                ...
                foreach ($ids as $key) {
                    ...
                    if (!$peer) {
                        ...
                        if (!$item->getFromDB($key)) {
                            ...
                            if ($linked_item && $linked_item->getFromDB($key)) {
                                if ($item_number == 1) {
                                    $item_1_tmp = $linked_item;
                                    $item_2_tmp = $peer;
                                } else {
                                    $item_1_tmp = $peer;
                                    $item_2_tmp = $linked_item;
                                }
                                if (!$item->getFromDBForItems($item_1_tmp, $item_2_tmp)) {
                                    if (
                                        ($specificities['check_both_items_if_same_type'])
                                        && ($item_1_tmp->getType() == $item_2_tmp->getType())
                                    ) {
                                        $item->getFromDBForItems($item_2_tmp, $item_1_tmp);
                                    }
                                }
                            }
                        }
                        ...
                        $criteria = [
                            ...
                            'WHERE'  => [
                                static::getIndexName() => $item->getID(),
                            ],
                        ];
                    }
